...
import NeoSelect from '@/components/NeoSelect'; // ⬅️ novo
...
export default function Home() {
  ...
  async function handleSelectNeo(id: string) {
    ...
    try {
      setLoadingNeo(true);
      const res = await fetch(`/api/neos/${id}`); // sua rota que consulta NeoWs
      ...
      setFormData((prev) => ({
        ...
        diameter: diameterM,
        velocity: velocityMs,
        // se quiser: usar densidade (g/cm³) vinda do SBDB (se sua rota retornar)
        // density: neo?.density_g_cm3 ? neo.density_g_cm3 * 1000 : prev.density,
      }));
