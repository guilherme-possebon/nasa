...
class FireCalculator {
    ...
    constructor(apiData: {
        ...
        this.asteroidName = apiData.name; // e.g., "(2004 SU55)" [cite: 2]
        ...
        this.velocityKps = parseFloat(apiData.close_approach_data[0].relative_velocity.kilometers_per_second); // e.g., 7.97 kps 
    }
    ...
    public getFireSpreadRadiusKm(): number {
        ...
        const radiusMetersResult = Math.sqrt(thermalEnergyJoules / (4 * Math.PI * IGNITION_THRESHOLD_J_M2));

        console.log(`Calculation for asteroid ${this.asteroidName} complete.`);
